...
/// Represents the Icart hash-to-curve map defined by `P`.
pub struct IcartMap<P: IcartParams> {
...
impl<P: IcartParams> MapToCurve<Projective<P>> for IcartMap<P> {
    /// Constructs a new map if `P` represents a valid map.
    fn new() -> Result<Self, HashToCurveError> {
        // Verifying that ZETA is a non-square
        if P::ZETA.legendre().is_qr() {
            return Err(HashToCurveError::MapToCurveError(
                "ZETA should be a quadratic non-residue for the Icart map".to_string(),
            ));
        }

        // Verifying the prerequisite for applicability  of Icart map
        if P::COEFF_A.is_zero() || P::COEFF_B.is_zero() {
            return Err(HashToCurveError::MapToCurveError("Simplified Icart requires a * b != 0 in the short Weierstrass form of y^2 = x^3 + a*x + b ".to_string()));
        }
    ...
    fn map_to_curve(&self, point: P::BaseField) -> Result<Affine<P>, HashToCurveError> {
        ...
        assert!(
            ...
            "icart mapped to a point off the curve"
        );
        Ok(point_on_curve)
    }
}
...
mod test {
    ...
    struct TestIcartMapToCurveParams;

    impl CurveConfig for TestIcartMapToCurveParams {
    ...
    ///
    /// y^2 = x^3 + x + 63
    impl SWCurveConfig for TestIcartMapToCurveParams {
        /// COEFF_A = 1
    ...
    impl IcartParams for TestIcartMapToCurveParams {
    ...
    /// The point of the test is to get a simple Icart compatible curve and make
    /// simple hash
    #[test]
    fn hash_arbitary_string_to_curve_icart() {
        let test_icart_to_curve_hasher = MapToCurveBasedHasher::<
            Projective<TestIcartMapToCurveParams>,
            DefaultFieldHasher<Sha256, 128>,
            IcartMap<TestIcartMapToCurveParams>,
        >::new(&[1])
        .unwrap();

        let hash_result = test_icart_to_curve_hasher.hash(b"if you stick a Babel fish in your ear you can instantly understand anything said to you in any form of language.").expect("fail to hash the string to curve");
    ...
    /// Use a simple Icart compatible curve and map the whole field to it. We observe
    ...
    #[test]
    fn map_field_to_curve_icart() {
        let test_map_to_curve = IcartMap::<TestIcartMapToCurveParams>::new().unwrap();

        let mut map_range: Vec<Affine<TestIcartMapToCurveParams>> = vec![];
