...
/// Trait defining the necessary parameters for the SwiftEC hash-to-curve method
/// for the curves of Weierstrass form of:
/// y^2 = x^3 + a*x + b where ab != 0. From [\[WB2019\]]
///
/// - [\[WB2019\]] <https://eprint.iacr.org/2019/403>
pub trait SwiftECParams: SWCurveConfig {
...
/// Represents the SwiftEC hash-to-curve map defined by `P`.
pub struct SwiftECMap<P: SwiftECParams> {
...
impl<P: SwiftECParams> MapToCurve<Projective<P>> for SwiftECMap<P> {
    /// Constructs a new map if `P` represents a valid map.
    fn new() -> Result<Self, HashToCurveError> {
        // Verifying that ZETA is a non-square
        if P::ZETA.legendre().is_qr() {
            return Err(HashToCurveError::MapToCurveError(
                "ZETA should be a quadratic non-residue for the SwiftEC map".to_string(),
            ));
        }

        // Verifying the prerequisite for applicability  of SwiftEC map
        if P::COEFF_A.is_zero() || P::COEFF_B.is_zero() {
            return Err(HashToCurveError::MapToCurveError("Simplified SwiftEC requires a * b != 0 in the short Weierstrass form of y^2 = x^3 + a*x + b ".to_string()));
        }
...
mod test {
    ...
    struct TestSwiftECMapToCurveParams;

    impl CurveConfig for TestSwiftECMapToCurveParams {
    ...
    ///
    /// y^2 = x^3 + x + 63
    impl SWCurveConfig for TestSwiftECMapToCurveParams {
        /// COEFF_A = 1
    ...
    impl SwiftECParams for TestSwiftECMapToCurveParams {
    ...
    /// The point of the test is to get a simple SwiftEC compatible curve and make
    /// simple hash
    #[test]
    fn hash_arbitary_string_to_curve_swiftec() {
        let test_swiftec_to_curve_hasher = MapToCurveBasedHasher::<
            Projective<TestSwiftECMapToCurveParams>,
            DefaultFieldHasher<Sha256, 128>,
            SwiftECMap<TestSwiftECMapToCurveParams>,
        >::new(&[1])
        .unwrap();

        let hash_result = test_swiftec_to_curve_hasher.hash(b"if you stick a Babel fish in your ear you can instantly understand anything said to you in any form of language.").expect("fail to hash the string to curve");
    ...
    /// Use a simple SwiftEC compatible curve and map the whole field to it. We observe
    ...
    #[test]
    fn map_field_to_curve_swiftec() {
        let test_map_to_curve = SwiftECMap::<TestSwiftECMapToCurveParams>::new().unwrap();

        let mut map_range: Vec<Affine<TestSwiftECMapToCurveParams>> = vec![];
